...
export function draw(equations, useEval, thetaDivId, velocityDivId, vars, cref1, cref2) {
  ...
  let canvas;
  if (useEval){
    canvas = findDOMNode(cref1);
    } else {
    canvas = findDOMNode(cref2); 
    }
  ...
  let radius, g, k, angle, velocity, omega, simSpeed, graphUpdateInterval, graphLen, project, wrap, trailLen;
  let varArr = [radius, g, k, angle, velocity, omega, simSpeed, graphUpdateInterval, graphLen, project, wrap, trailLen] = vars;
  for (let i = 0; i < vars.length; i++){
    if (varArr[i] === 'true' || varArr[i] === 'true'){
      varArr[i] = Boolean(varArr[i]);
    } else{
      varArr[i] = Number(varArr[i]);
    }
  }
  [radius, g, k, angle, velocity, omega, simSpeed, graphUpdateInterval, graphLen, project, wrap, trailLen] = varArr;
